...
<head>
    ...
    <title>Private Chats - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">Private Chats</h1>
...
<article>
    <h2>Private Chats</h2><p>A user may want to make a private chat no other users can access. To do this,
create a new chat with the syntax:</p>
<pre class="prettyprint source"><code>let privChat = new Chat('private', true);</code></pre><p>The second parameter tells ChatEngine to hit the <code>/chat</code> endpoint which
...
with the same channel.</p>
<h2>Inviting to Private Chats</h2><p>In order to securely invite other users to the chat, the client can call
the <code>invite</code> method.</p>
<pre class="prettyprint source lang-js"><code>otherUser = ChatEngine.global.users['ian'];
privChat.invite(otherUser);</code></pre><p>This will send <code>otherUser</code> a secure invite to the <a href="Chat.html">Chat</a> via <a href="User.html#direct">User#direct</a>.</p>
<h2>Receiving invites to Private Chats</h2><p>You can get notified of invites by subscribing to the <code>'$.invite</code> event.</p>
