...
<head>
    ...
    <title>Event Payload - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">Event Payload</h1>
...
<article>
    <h2>Event Payload</h2><p>Although you may emit any valid JSON using <a href="Chat.html#emit">Chat#emit</a>, when the same
event is received by other <a href="User.html">User</a>s, it will be augmented with additional
data.</p>
...
}</code></pre><p>You can find the actual message contents supplied to <a href="Chat.html#emit">Chat#emit</a> within the <code>payload.data</code> property.</p>
<p>ChatEngine event payloads are augmented with additional information supplied by the framework. Most of the time these are <code>payload.sender</code> and ```payload.chat.</p>
<p>The property <code>payload.chat</code> is the <a href="Chat.html">Chat</a> that event was broadcast broadcast on, and the <code>payload.sender</code> is the <a href="User.html">User</a> that broadcast the message via <a href="Chat.html#emit">Chat#emit</a>.</p>
<p>The <a href="User.html">User</a> and <a href="Chat.html">Chat</a> properties are both fully interactive instances. Therefor, you can do things like <code>payload.chat.emit('message')</code> to automatically reply to a message.</p>
<h2>Simple Example</h2><p>In this demo we'll mock up a user named 'Ian' emitting the 'like' event on a user named 'Emily'.</p>
<p>On Ian's page:</p>
<pre class="prettyprint source lang-js"><code>// connect with UUID 'ian' and add a user state
ChatEngine.connect('ian', {fullName: 'Ian Jennings'});

// emit a 'like' event over global chat
ChatEngine.global.emit('like', {
    who: 'emily'
});</code></pre><p>On Emily's page:</p>
<pre class="prettyprint source lang-js"><code>// connect with UUID 'emily'
