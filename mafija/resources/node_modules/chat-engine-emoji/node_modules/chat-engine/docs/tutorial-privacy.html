...
<head>
    ...
    <title>Privacy - PubNub ChatEngine</title>
...
<article>
    <h2>Auth Url</h2><p>ChatEngine uses PubNub Functions to handle <a href="Chat.html">Chat</a> privacy.</p>
<p>When the <a href="ChatEngine.html#connect">ChatEngine#connect</a> method is invoked, ChatEngine will make an authentication request to <code>ceConfig.authUrl</code>. This url is responsible for granting the uuid permission to access the network.</p>
<h3>Insecure Mode</h3><p><strong>If <code>ceConfig.authUrl</code> is omitted, the app is run in <code>insecure</code> mode</strong>.</p>
<p>In insecure mode, all <a href="User.html">User</a>s can access all <a href="Chat.html">Chat</a>s and nothing is private (none of the below applies). This is not recommended and a warning will be logged to console.</p>
<h2>Chat Security</h2><h3>Public Chat</h3><pre class="prettyprint source lang-js"><code>new Chat('channel', false);</code></pre><p>This is a public chat any <a href="User.html">User</a> can join.</p>
<h3>Private Chat</h3><pre class="prettyprint source lang-js"><code>new Chat('channel', true);</code></pre><p>This is a private chat that a user must authenticated in. Usually this is done via <a href="Chat.html#invite">Chat#invite</a>.</p>
<h2>Direct Chat</h2><p><a href="User.html#direct">User#direct</a> is a <a href="Chat.html">Chat</a> that any <a href="User.html">User</a> can <a href="Chat.html#emit">Chat#emit</a> on, but only the user receives events on.</p>
<p>This is helpful for sending messages directly to users, to ping them, or challenge them to a match. This channel is only readable by said user.</p>
<pre class="prettyprint source lang-js"><code>// me
me.direct.on('game-invite', (payload) -> {
     console.log(payload.sender.uuid, 'sent your a game invite on the map', payload.data.map);
});

// someone else
them.direct.emit('game-invite', {map: 'de_dust'});</code></pre><h2>Feed Chat</h2><p><a href="User.html#feed">User#feed</a> is a <a href="Chat.html">Chat</a> that only the <a href="User.html">User</a> can <a href="Chat.html#emit">Chat#emit</a> to but everyone can receive events on.</p>
<p><a href="User.html">User</a>s can use this to tell others of their status. This is useful for things like updating a temporary status (&quot;user is typing...&quot;) or letting others know you've gone idle.</p>
<p>For a more persistent status update, see the section in this tutorial on &quot;state&quot; and <a href="User.html#state">User#state</a>.</p>
<pre class="prettyprint source lang-js"><code>// me
...
});</code></pre><h2>PubNub Security</h2><p>Security is controlled via <a href="https://www.pubnub.com/docs/tutorials/pubnub-access-manager">PubNub PAM</a>.</p>
</article>
