...
<head>
    ...
    <title>Global Chat - PubNub ChatEngine</title>
...
<article>
    <p>ChatEngine.global is a global <a href="Chat.html">Chat</a> room that all instances of <a href="ChatEngine.html">ChatEngine</a> connect to by default.
It is a convenience property that provides some handy utility.</p>
<p>It makes it easy to send a message to all connected clients:</p>
<pre class="prettyprint source"><code>ChatEngine.global.send('message', 'Sale going on now!');</code></pre><p>We also get a list of all online users connected to this instance of ChatEngine.</p>
<pre class="prettyprint source lang-js"><code>// everyone online in all chatrooms
console.log(ChatEngine.global.users);</code></pre><p>So how does it work?</p>
<ul>
<li>All <a href="User.html">User</a>s connect to ChatEngine.global by default (during <code>connect()</code>)</li>
<li>User.state() and Me.update() use ChatEngine.global as the default input. See: <a href="tutorial-users.html">Users and State</a></li>
</ul>
</article>
