...
<head>
    ...
    <title>History - PubNub ChatEngine</title>
...
<article>
    <p>History is a way to retrieve old events that were fired before ChatEngine
was loaded. This way, when someone reloads their page or closes the app,
they'll see the old messages the next time they log in.</p>
<p>You can trigger history events by calling the <a href="Chat.html#history">Chat#history</a> method with
the event you'd like to retrieve as the first command.</p>
<pre class="prettyprint source lang-js"><code>chat.history('message');</code></pre><p>This will fire old events on the <a href="Chat.html">Chat</a>. The events will have the same
payload (reprocessed by plugins at runtime) and emitted with the the
<code>$.history</code> prefix. See <a href="Chat.html#event:$%2522.%2522history%2522.%2522*">Chat#event:$"."history"."*</a>.</p>
<p>The above call triggers the <code>$.history.message</code> event
and the events can be subscribed to via:</p>
<pre class="prettyprint source lang-js"><code>chat.on('$.history.message', (payload) => {});</code></pre><p>Altogether it looks like:</p>
<pre class="prettyprint source lang-js"><code>// when this chat gets a message
