...
<head>
    ...
    <title>Templating - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">Templating</h1>
...
<article>
    <p>ChatEngine does not contain any premade templates or GUI code as part of the
main package (they're are plenty in the example repository however).</p>
<p>ChatEngine leaves the design and interaction up to you. It provides events,
methods, and data but does not make any assumptions of template engines.</p>
<p>ChatEngine works on both the front and back ends, with popular frameworks like
jQuery, React, Angular, React Native, NodeJS, and good old Javascript.</p>
<p>For example, to publish a message to a <a href="Chat.html">Chat</a> you may bind a DOM <code>onSubmit()</code> event to
the <a href="Chat.html#emit">Chat#emit</a> method.</p>
<p>So, let's create a chat:</p>
<pre class="prettyprint source lang-js"><code>// create a new chat
let chat = new ChatEngine.Chat('example-chat');</code></pre><p>Create a place for message  output and give the user a way to fire the event:</p>
<pre class="prettyprint source lang-html"><code>&lt;div id=&quot;output&quot;>&lt;/div>
&lt;button type=&quot;submit&quot; onclick=&quot;sendChat()&quot;>Send&lt;/button></code></pre><p>The <code>sendChat()</code> method fires <a href="Chat.html#emit">Chat#emit</a>.</p>
<pre class="prettyprint source lang-js"><code>// send a message to all users in the chat
function sendChat() {
...
};</code></pre><p>Then, when the message is recieved by the client:</p>
<pre class="prettyprint source lang-js"><code>// get the DOM element in Javascript
...
});</code></pre><h2>Data Binding (Angular and React)</h2><p>ChatEngine has a local data system that keeps data about <a href="Chat.html">Chat</a>s and <a href="User.html">User</a>s in sync. So when working with data bound system we get some functionality for free!</p>
<p>For example in angular:</p>
<pre class="prettyprint source"><code>&lt;ul id=&quot;online-list&quot;>
    &lt;li ng-repeat=&quot;(uuid, user) in chat.users&quot;>
        &lt;a href=&quot;#&quot; ng-click=&quot;chat.invite(user)&quot;>{{user.state().username}}&lt;/a>
    &lt;/li>
&lt;/ul></code></pre><p>Note that binding ChatEngine data to Angular requires forcefully telling Angular that something has changed. The following is a small Angular plugin that, when loaded, will render your Angular app anytime anything changes in ChatEngine.</p>
<pre class="prettyprint source lang-js"><code>// plugin
