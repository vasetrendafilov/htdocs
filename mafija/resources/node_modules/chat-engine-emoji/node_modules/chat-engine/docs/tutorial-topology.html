...
<head>
    ...
    <title>PubNub Channel Topology - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">PubNub Channel Topology</h1>
...
<header>
    ...
    <!-- <h2>PubNub Channel Topology</h2> -->
</header>

<article>
    <p>PubNub Channels in ChatEngine</p>
<h2>Global Chat / Channel</h2><p>By default the Global Channel is called <code>chat-engine</code>.</p>
<p>Every <a href="Chat.html">Chat</a> created within an instanced of <a href="ChatEngine.html">ChatEngine</a> is perpended with
the string <code>chat-engine#</code>.</p>
<p>This allows you to create multi-tenant apps without overlap. For example, you may want to
support multiple organizations with different top level ids. Every organization would have
a unique global channel.</p>
<p>Every channel created within ChatEngine thereafter is prepended with the global channel.</p>
<h2>Channel Namespaces</h2><p>Every <a href="Chat.html">Chat</a>s is created under a sub namespace dependent on the context and permissions.</p>
<h4><code>globalChannel</code></h4><p>This is the channel for ChatEngine#global. All <a href="User.html">User</a>s have read and write access to this channel.</p>
<pre class="prettyprint source lang-js"><code>ChatEngine.global.on('message', () => {});

console.log(ChatEngine.global.channel);
// ChatEngine.global.channel == &quot;chat-engine&quot;</code></pre><h4><code>globalChannel + '#chat#public.*'</code></h4><p>All public <a href="Chat.html">Chat</a>s are created under this namespace. All <a href="User.html">User</a>s have read and write access to this channel.</p>
<pre class="prettyprint source lang-js"><code>let chat = new Chat('custom-channel')

console.log(chat.channel);
// chat.channel == &quot;chat-engine#chat#public.#custom-channel&quot;</code></pre><h4><code>globalChannel + '#chat#private.*'</code></h4><p>All private <a href="Chat.html">Chat</a>s are create under this namespace. This namespace is locked down and no users have
read or write access. They must be granted them.</p>
<pre class="prettyprint source lang-js"><code>let privateChat = new Chat('private-channel', true);

console.log(privateChat.channel);
// privateChat.channel == &quot;chat-engine#chat#private.#private-channel&quot;</code></pre><h4><code>globalChannel + '#user#' + myUUID + '#read.*'</code></h4><p>This is the namespace containing <a href="User.html">User</a> owned <a href="Chat.html">Chat</a>s. The user who's <a href="User.html#uuid">User#uuid</a> matches <code>myUUID</code> has all permissions, while other <a href="User.html">User</a>s only have read permissions. <a href="User.html#feed">User#feed</a> belongs to this namespace.</p>
<pre class="prettyprint source lang-js"><code>let joe = new ChatEngine.User('joe');

console.log(joe.feed.channel);
// joe.feed.channel == &quot;chat-engine#user#joe#read#feed&quot;</code></pre><h4><code>globalChannel + '#user#' + myUUID + '#write.*'</code></h4><p>This is the namespace containing <a href="User.html">User</a> owned <a href="Chat.html">Chat</a>s. The user who's <a href="User.html#uuid">User#uuid</a> matches <code>myUUID</code> has all permissions, while other <a href="User.html">User</a>s only have write permissions. <a href="User.html#direct">User#direct</a> belongs to this namespace.</p>
<pre class="prettyprint source lang-js"><code>let joe = new ChatEngine.User('joe');

console.log(joe.direct.channel);
// joe.direct.channel == &quot;chat-engine#user#joe#write#direct&quot;</code></pre>
</article>
