...
<head>
    ...
    <title>Wildcards - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">Wildcards</h1>
...
<article>
    <p>You can get notified of every event that a ChatEngine object emits by subscribing to the <code>*</code> wildcard event or using the <code>onAny</code> method.</p>
<pre class="prettyprint source lang-js"><code>Chat.on('*', (event, payload) => {
    console.log('something happened', event, payload);
});
Chat.onAny((event, payload) => {
    console.log('something happened', event, payload);
});</code></pre><h3>Namespaced Wildcards</h3><p>Wildcards also work within namespaces. For example, you could subscribe to all system events with <code>$.*</code> or get notified of all plugin events by subscribing to <code>$plugin.*</code>.</p>
<h3>All Events Everywhere</h3><p>All events any object in ChatEngine fires is also emitted from the ChatEngine object. You can get notified of every event by subscribing to <a href="ChatEngine.html#onAny">ChatEngine#onAny</a>.</p>
<p>This is helpful for debugging and notifying frameworks like Angular when the GUI needs to be updated.</p>
<pre class="prettyprint source lang-js"><code>ChatEngine.onAny(function(event, payload) {
    console.log('something happened', event, payload);
});</code></pre>
</article>
