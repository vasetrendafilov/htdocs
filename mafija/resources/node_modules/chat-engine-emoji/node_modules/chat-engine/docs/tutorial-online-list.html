...
<head>
    ...
    <title>Online List - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">Online List</h1>
...
<article>
    <h3>Online List</h3><p><img src="/getting-started/assets/README-c71c143b.png" alt=""></p>
<p>A list of all the clients who have joined the chatroom is available from <code>chat.users</code>.</p>
<pre class="prettyprint source lang-js"><code>console.log(chat.users);</code></pre><p>It returns a list of <code>Users</code> who have also joined this chat.</p>
<pre class="prettyprint source lang-js"><code>{
  ian: {},
  nick: {}
}</code></pre><p>This property is kept in sync as Users join and leave the Chats. If you're using
a library like React or Angular, you can simply loop through <code>chat.users</code>
to render list of online users.</p>
<h3>Online Events</h3><p>The list of</p>
<ul>
<li>When first joining a Chat, the Chat will emit <code>$.online.here</code> for every <code>User</code> in the room.</li>
<li>Any time a new <code>User</code> joins, the <code>Chat</code> emits the <code>$.online.join</code> event.</li>
</ul>
<p>Here we subscribe both events with wildcard syntax (<code>$.online.*</code>).</p>
<pre class="prettyprint source lang-js"><code>chat.on('$.online.*', (newUser) -> {
  console.log('new user', newUser);
});</code></pre><h3>Hooking it up to a GUI</h3><p>Let's combine the information above into a small app that logs when you and other users come online.</p>
<p><img src="/guide/getting-started/assets/README-c71c143b.png" alt=""></p>
<p>First, we'll create a function to log messages into HTML.</p>
<p>Add the following to the <code>&lt;body&gt;</code> of <code>index.html</code> to build a place-holder for our log.</p>
<pre class="prettyprint source lang-html"><code>&lt;div class=&quot;container&quot;>
  &lt;div class=&quot;row&quot;>
      ...
      &lt;/div>
  &lt;/div>
&lt;/div></code></pre><p>Next, we'll create a function that adds  <code>username: text</code> as a line in the log.</p>
<pre class="prettyprint source lang-js"><code>const appendMessage = (username, text) => {
...
};</code></pre><p>Then, listen for the <code>$.ready</code> event to find out when the client is connected to the <code>Chat</code>.</p>
<pre class="prettyprint source lang-js"><code>chat.on('$.ready', (payload) => {
  appendMessage('Status', 'Connected to chat!');
});</code></pre><p>We subscribe to the <code>$.online.*</code> event to learn about online <code>User</code>s.</p>
<pre class="prettyprint source lang-js"><code>chat.on('$.online.*', (payload) => {
  appendMessage('Status', payload.user.uuid + ' is in this room!');
});</code></pre><p>You should see a message showing that <code>ian</code> has come online and that connection has been established.</p>
<p><img src="/guide/getting-started/assets/README-c71c143b.png" alt=""></p>
</article>
