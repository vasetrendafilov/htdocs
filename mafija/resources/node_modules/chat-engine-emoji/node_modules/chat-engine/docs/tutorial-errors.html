...
<head>
    ...
    <title>Errors - PubNub ChatEngine</title>
...
<article>
    <h3>Errors Two Ways</h3><p>When developing with ChatEngine, errors will occasionally be reported to let you know when things go wrong.</p>
<p>By default, errors are both thrown from ChatEngine source and emitted as <code>$.error</code> events from the object that emitted them.</p>
<h4>Thrown Errors</h4><p>You'll notice thrown errors because they usually break Javascript execution and log a stack trace within the console.</p>
<h4>Errors as Events</h4><p>You can also subscribe to errors via the <code>$.error</code> event. If you'd like to subscribe to all ChatEngine errors, try the following:</p>
<pre class="prettyprint source lang-js"><code>ChatEngine.onAny('$.error.*', (payload) {
   console.log(payload);
});</code></pre><h4>Errors in Production</h4><p>In a production app, it is not a good idea to throw errors. If you'd like to suprress errors, supply <code>throwErrors: false</code> in your <code>ceConfig</code>.</p>
<h3>Example Errors</h3><p>These are the errors thrown when a client tries to access a <a href="Chat.html">Chat</a> you don’t have PAM access to (see privacy):</p>
<pre class="prettyprint source lang-js"><code>let PrivateChat = new ChatEngine.Chat(‘locked-down-i-dont-have-permissions’);</code></pre><p>Private Chat Emits These Events</p>
<ul>
<li>$.error.presence</li>
<li>$.error.publish</li>
</ul>
<p>ChatEngine emits this event:</p>
<ul>
<li>$.network.denied</li>
</ul>
</article>
