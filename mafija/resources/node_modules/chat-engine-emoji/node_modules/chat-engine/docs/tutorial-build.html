...
<head>
    ...
    <title>Build a Plugin - PubNub ChatEngine</title>
...
<div id="main">
    ...
    <h1 class="page-title">Build a Plugin</h1>
...
<article>
    <h1>Plugin.js</h1><p>Check out <a href="module-chat-engine-markdown.html">module:chat-engine-markdown</a> and <a href="module-chat-engine-typing-indicator.html">module:chat-engine-typing-indicator</a> for some examples of plugins.</p>
<h2>What is a pugin?</h2><p>A plugin is a NodeJS module that exports a mixin like object that augments ChatEngine objects.</p>
<h1>Plugin Anatomy</h1><p>The plugin entry file must be a file called <code>plugin.js</code> in the root directory.
From this file you can require any other file as normal, but the entry must be
plugin.js</p>
<p>Every plugin must return an object containing the property <code>namespace</code> and <code>middleware</code>
or <code>extends</code>.</p>
<p>You can also ChatEngine objects and add new methods to them. For example,
this plugin adds a method called <code>newMethod()</code> to the <a href="Chat.html">Chat</a> class.</p>
<pre class="prettyprint source lang-js"><code>// newmethod/plugin.js
module.exports - {
    return {
        extends: {
            Chat: {
...
}</code></pre><p>Then you can attach the plugin to a <a href="Chat.html">Chat</a> like this:</p>
<pre class="prettyprint source lang-js"><code>// include the plugin via require
...
// Console: 'New Method Fired - this Chat object is available as this.parent</code></pre><p>When the plugin is installed, every instance of <code>ChatEngine.Chat</code> will have a new
method called <code>newMethod()</code>. You can call the method like <code>someChat.newMethod()</code>.</p>
<h1>Middleware</h1><p>Middleware allows you to transform payloads as they travel through the system.
They are executed in order they are assigned.</p>
<p>The only valid properties of the <code>middleware</code> object are <code>send</code> and <code>broadcast</code>.</p>
<ul>
<li><code>send</code> is executed before the payload is sent over the
network to the rest of the connected clients.</li>
<li><code>broadcast</code> is executed when the client receives a payload from another
client.</li>
</ul>
<pre class="prettyprint source lang-js"><code>// timer/plugin.js
...
}</code></pre><p>The sub properties under <code>send</code> and <code>broadcast</code> are the events
that will trigger the transformation.</p>
<p>For  example, the plugin above will be executed when a <code>message</code>
event is sent from the client.</p>
<pre class="prettyprint source lang-js"><code>// include the plugin via require
