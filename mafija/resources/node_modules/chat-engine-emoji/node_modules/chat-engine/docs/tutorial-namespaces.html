...
<head>
    ...
    <title>Namespaces - PubNub ChatEngine</title>
...
<article>
    <h2>Namespaces</h2><p>Event names follow a pattern that helps determine where an object originated.</p>
<h3>Custom Events</h3><p>Custom events are events you define in your framework, like <code>message</code> and <code>invite</code>. They are simply strings, but they should not include any special characters (except <code>.</code>).</p>
<h3>$ - System Events</h3><p>System events always begin with a <code>$</code>. <code>$.ready</code> and <code>$.online</code> are examples of events emitted by ChatEninge. They are system events that are automatically emitted when specific things happen. System events are documented in the reference.</p>
<h3>$plugin - PLugin Events</h3><p>Plugin events always begin with <code>$pluginName</code>. <code>$typingIndicator.startTyping</code> is an example of an event emitted by a plugin. The <code>$typingIndicator</code> string is a plugin namespace and <code>startTyping</code> is the plugin event. Namespacing plugins helps ensure that there is no collision between plugins.</p>
<h3>Event Children</h3><p>Dots (<code>.</code>) in an event name indicate that the event is a child of some parent. For example <code>image.like</code> indicates that a &quot;like&quot; event was fired for a specific &quot;image&quot;. You could then subscribe to all <code>image</code> events by subscribing to the <code>image.*</code> event. See more about wildcards below.</p>
</article>
